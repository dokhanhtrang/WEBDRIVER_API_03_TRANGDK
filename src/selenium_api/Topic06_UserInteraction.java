...
public class Topic06_UserInteraction {
	...
	@Test
	public void TC02_ClickandHold_SelectMultiple() throws Exception {
		...
		List<WebElement> numberSelected = driver
				.findElements(By.xpath("//li[@class='ui-state-default ui-selecteeui-selected']"));
		Assert.assertEquals(4,numberSelected.size());
		System.out.println("Done TC 02");
	}
...
//		hover.click();
	...
	public void TC05_DragAndDrop() throws Exception  {
		...
		Action dragAndDrop1 = builder1.clickAndHold(dragFrom1).moveToElement(target1).release(target1).build();
		dragAndDrop1.perform();
		Thread.sleep(1000);
		Assert.assertTrue(target1.getText().contains("Dropped!"));
		System.out.println("Done TC 05");
	}
